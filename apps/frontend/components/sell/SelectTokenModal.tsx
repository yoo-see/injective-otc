  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white w-[423px] h-[371px] rounded-[20px] left-0 top-0"
      >
        <div className="flex flex-row justify-between items-center w-[423px] h-[62px] px-6 py-4 left-0 top-0">
          ...
          <SvgIcon.Close
            onClick={handleModalFunc}
            className="hover:cursor-pointer hover:bg-[#b6b8ff]"
          />
        </div>
        <div className="block h-[300px] rounded-[20px] gap-7 px-6 py-4 overflow-y-scroll scrollbar scrollbar-w-2 scrollbar-h-5 scrollbar-thumb-[#959595] scrollbar-track-none scrollbar-thumb-rounded ">
          {tokens?.map((ele, idx) => {
            return (
              <div key={`${ele.token} + ${idx}`} className="flex">
                <div
                  onClick={getTokenSelect}
                  className="flex flex-row items-center gap-2 py-1 px-3 mb-4 rounded-[20px] hover:bg-[#b6b8ff] hover:cursor-pointer"
                >
                  <SvgIcon.Ether />
